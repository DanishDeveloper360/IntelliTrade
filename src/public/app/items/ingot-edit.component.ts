...
export class IngotEditComponent implements OnInit {
  ...
  calculateData(wasteZincCalculation?: boolean) {
    if(wasteZincCalculation)
    {  
      this.CalculateZinc(null);
      this.calculateWaste(null);
    }
    
    this.ingot.totalItemWeight =
      this.ingot.items
        .map(x => {
          return x.weight;
        })
    ...
    this.ingot.totalItemWeightMinusZinc = this.ingot.items
      .map(x => {
        return x.weight;
      })
      .reduce((a, b) => a + b, 0);

    this.ingot.netIngotWeight =
      this.ingot.totalItemWeight -
      this.ingot.lm -
      this.ingot.sisa -
      this.ingot.rodi -
  ...
  calculateWaste(event: Event) {
    let wasteWeight = this.ingot.items
      .map(x => {
  ...
  CalculateProfit(event) {
    ...
    let totalItemRate = this.ingot.items
      .map(x => {
    ...
    let rodiRate =
      this.items.find(x => x.name.toLowerCase().trim() == "rodi").rate *
      this.ingot.rodi;
    let lmRate =
      this.items.find(x => x.name.toLowerCase().trim() == "lm").rate *
      this.ingot.lm;
    let sisaRate =
      this.items.find(x => x.name.toLowerCase().trim() == "sisa").rate *
      this.ingot.sisa;
    let lohaRate =
      this.items.find(x => x.name.toLowerCase().trim() == "loha").rate *
      this.ingot.loha;

    this.ingot.profit =
      -10000 +
      this.ingot.netIngotWeight * this.ingot.ingotRate -
      totalItemRate +
      rodiRate +
      lmRate +
      sisaRate +
      lohaRate -
      this.ingot.zinc * this.ingot.zincRate;
  }

  CalculateZinc(event: Event) {
    let zincWeight = this.ingot.items
      .map(x => {
